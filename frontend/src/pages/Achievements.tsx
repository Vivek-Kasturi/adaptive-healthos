...
export default function Achievements({ userId }: Props) {
  ...
  useEffect(() => {
    getGamificationState(userId)
      .then(r => {
        if (r.data) {
          // Always normalize — MongoDB doc may be missing fields
          setGame({
    ...
    <div className="space-y-4">
      ...
      <div className="bg-gradient-to-br from-green-900/40 to-gray-900 border border-green-500/30 rounded-2xl p-5">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <h3 className="text-4xl font-black text-green-400">{level}</h3>
          </div>
          <div className="text-right">
            <p className="text-gray-400 text-sm">Total XP</p>
            <p className="text-white font-bold text-2xl">{xpTotal}</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 text-center">
          ...
          <div className="text-orange-400 text-2xl font-bold">{game.current_streak_days ?? 0}</div>
          <div className="text-gray-400 text-xs">Day Streak</div>
          <div className="text-gray-600 text-xs mt-1">Best: {game.longest_streak_days ?? 0} days</div>
        </div>
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 text-center">
          <div className="text-3xl mb-1">⚡</div>
          <div className="text-yellow-400 text-2xl font-bold">{game.weekly_xp ?? 0}</div>
          <div className="text-gray-400 text-xs">This Week's XP</div>
          <div className="text-gray-600 text-xs mt-1">{achievements.length} achievements</div>
        </div>
      </div>
      ...
      <div>
        ...
        <div className="space-y-2">
          {ALL_ACHIEVEMENTS.map(a => {
            ...
            const unlockedData = achievements.find((u: any) => String(u.id || u) === a.id)
            return (
              <div key={a.id} className={`flex items-center gap-3 rounded-xl p-3 border transition-all
                ...
                <div className="flex-1">
                  ...
                  {unlocked && unlockedData && typeof unlockedData === 'object' && unlockedData.unlocked_at && (
